...
# 使い方:
#   python3 tag.py 写真のパス [--status protected|lost]
#   例: python3 tag.py dog.jpg --status protected
...
def extract_tags(image_path: str):
    ...
    if message.content is None:
        # Kimi-K2.6のような推論系モデルは、先に思考過程(reasoning)を生成してから
        # 最終回答(content)を出すため、max_tokensが足りないとcontentが空のまま
        # 打ち切られることがある。診断のためfinish_reasonと生レスポンスを表示する。
